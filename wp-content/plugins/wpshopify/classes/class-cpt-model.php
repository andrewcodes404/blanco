...
class CPT_Model
{
	...
	*/
    public function set_existing_collection_post_id(
        $collection_model,
        $all_collections,
        $collection
    ) {
        $existing_post_id = CPT::find_existing_post_id_from_collection(
            $all_collections,
            $collection
        );
	...
	*/
    public function set_existing_product_post_id(
        $all_products,
        $product_model,
        $product
    ) {
        $existing_post_id = CPT::find_existing_post_id_from_product(
            $all_products,
            $product
        );
	...
	*/
    public function build_collections_model_for_update(
        $all_collections,
        $collection,
        $menu_order = 0
    ) {
        $collection_model = $this->set_collection_model_defaults($collection);
        $collection_model = $this->set_existing_collection_post_id(
            $collection_model,
            $all_collections,
            $collection
        );
	...
	*/
    public function build_products_model_for_update(
        $all_products,
        $product,
        $menu_order = 0
    ) {
        $product_model = $this->set_product_model_defaults($product);
        $product_model = $this->set_existing_product_post_id(
            $all_products,
            $product_model,
            $product
        );

        $post_id = isset($product_model['ID']) ? $product_model['ID'] : false;
	...
	*/
    ...
    {
        $all_products = CPT::get_all_posts_by_type(
            WP_SHOPIFY_PRODUCTS_POST_TYPE_SLUG
        );

        $model = $this->build_products_model_for_update(
            $all_products,
            $product,
            $menu_order
        );
	...
	*/
    public function insert_or_update_collection_post(
        ...
        $all_collections = CPT::get_all_posts_by_type(
