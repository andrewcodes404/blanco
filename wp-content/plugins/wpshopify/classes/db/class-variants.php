...
use WP_Shopify\Utils\Filtering as Utils_Filtering;
use WP_Shopify\Utils\Pricing;

if (!defined('ABSPATH')) {
    exit();
}

class Variants extends \WP_Shopify\DB
{
    ...
    {
        // Table info
        $this->table_name_suffix = WP_SHOPIFY_TABLE_NAME_VARIANTS;
   ...
   */
	...
	*/
    ...
    public function get_in_stock_variants_from_product($product)
    {
        $variants = $this->get_all_variants_from_post_id($product->post_id);

        return Utils_Sorting::sort_by(
            Utils::only_available_variants($variants),
            'position'
        );
    }

    public function get_in_stock_variants_from_product_id($product_id)
    {
        $variants = $this->get_variants_from_product_id($product_id);

        return Utils::only_available_variants($variants);
    }
	...
	*/
    public function get_variants_from_post_id_query()
    {
        ...
        return 'SELECT variants.* FROM ' .
            $wpdb->prefix .
            WP_SHOPIFY_TABLE_NAME_PRODUCTS .
            ' as products INNER JOIN ' .
            $this->table_name .
            ' as variants ON products.product_id = variants.product_id WHERE products.post_id = %d';
    }
	...
	*/
    ...
    {
        $found_variants = array_values(
            $this->find_first_non_zero_compare_at_price($variants)
        );
	...
	*/
    public function check_if_all_variant_prices_match(
        $last_variant_price,
        $first_variant_price
    ) {
    ...
    public function maybe_add_product_id_to_variants($maybe_products)
    {
        if (is_array($maybe_products)) {
            foreach ($maybe_products as $product) {
                $product->variants = $this->add_product_id_to_variants(
                    $product->variants,
                    $product
                );
            }
        } else {
            $maybe_products->variants = $this->add_product_id_to_variants(
                $maybe_products->variants,
                $maybe_products
            );
        }
