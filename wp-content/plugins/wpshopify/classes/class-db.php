...
class DB
{
	...
	*/
    public function maybe_rename_to_lookup_key($item)
    {
        ...
        if (!Utils::has($item, WP_SHOPIFY_SHOPIFY_PAYLOAD_KEY)) {
            return $item;
        }
	...
	*/
    public function get_row_by($column_name, $column_value)
    {
        ...
        return $this->sanitize_db_response(
            $results,
            'WP Shopify Error - Unable to get single database row.',
            'get_row'
        );
    }
   ...
   */
    public function select_in_query($options)
    {
        ...
        return 'SELECT items.' .
            $options['col_to_return'] .
            ' FROM ' .
            $this->table_name .
            ' as items WHERE items.' .
            $options['col_to_check'] .
            ' IN(' .
            $vals_prepared .
            ')';
    }

    public function select_like_query($options)
    {
        return 'SELECT items.' .
            $options['col_to_return'] .
            ' FROM ' .
    ...
    public function select_in($options)
    {
        ...
        $prepared_query = $wpdb->prepare(
            $this->select_in_query($options),
            $options['vals_to_check']
        );
    ...
    public function select_like($options)
    {
        ...
        $asdfsd = '%' . $wpdb->esc_like($options['vals_to_check']) . '%';

        $prepared_query = $wpdb->prepare(
            $this->select_like_query($options),
            $asdfsd
        );
   ...
   */
	...
	*/
    public function get_column($column_name, $column_value, $table_name = false)
    {
        ...
        return $wpdb->get_var($wpdb->prepare($query, $column_value));
    }
	...
	*/
    public function get_column_single($column)
    {
        ...
        if (!$this->table_exists($table_name)) {
            return $this->sanitize_db_response(
                false,
                'WP Shopify Error - Failed to get single database column. Table "' .
                    $table_name .
                    '" doesn\'t exist',
                'get_var'
            );
        ...
        if (!is_string($column)) {
            return $this->sanitize_db_response(
                false,
                'WP Shopify Error - Database column name is not a string',
                'get_var'
            );
        }
        ...
        if (!\array_key_exists($column, $this->get_columns())) {
            return $this->sanitize_db_response(
                false,
                'WP Shopify Error - Database column name does not exist. Please try reinstalling the plugin from scratch',
                'get_var'
            );
        }
        ...
        return $this->sanitize_db_response(
            $results,
            'WP Shopify Error - Database column name is not a string. Please clear the plugin cache and try again.',
            'get_results'
        );
    }
	...
	*/
    public function sanitize_db_response(
		...
		*/
        ...
        if (
            (is_array($result) && Utils::array_is_empty($result)) ||
            is_null($result)
        ) {
            return false;
        }
    ...
    public function insert($data)
    {
		...
		*/
        ...
        if ($this->has_existing_record($data)) {
            ...
            return $this->sanitize_db_response(
                $result,
                'Failed to update database record. Please clear the plugin cache and try again.',
                'update'
            );
        } else {
            ...
            return $this->sanitize_db_response(
                $result,
                'Failed to insert database record. Please clear the plugin cache and try again.',
                'insert'
            );
        }
    }
	...
	*/
    public function update_column_single(
        $data = [],
        $where = [],
        $formats = false
    ) {
        ...
        if (!$this->table_exists($table_name)) {
            return $this->sanitize_db_response(
                false,
                'Failed to update database record. Table does not exist.',
                'update'
            );
        }
        ...
        if (empty($where)) {
            return $this->sanitize_db_response(
                false,
                'Failed to update database record. Where clause does not exist.',
                'update'
            );
        }
	...
	*/
    public function truncate()
    {
        ...
        if (!$this->table_exists($table_name)) {
            return $this->sanitize_db_response(
                false,
                'WP Shopify Error - Tried to truncate table ' .
                    $table_name .
        ...
        return $this->sanitize_db_response(
            $query_results,
            'WP Shopify Error - Failed to truncate table ' .
                $table_name .
                '. Please clear the plugin cache and try again.',
            'query'
        );
    }
	...
	*/
    public function delete($row_id = 1)
    {
        ...
        if (!$this->table_exists($table_name)) {
            return $this->sanitize_db_response(
                false,
                'WP Shopify Error - Tried to perform deletion on table ' .
                    $table_name .
                    ' but table doesn\'t exist.',
                'get_var'
            );
        ...
        return $this->sanitize_db_response(
            $query_results,
            'WP Shopify Error - Failed to delete record(s) on table ' .
                $table_name .
                '. Please clear the plugin cache and try again.',
            'query'
        );
	...
	*/
    public function query($query)
    {
        ...
        return $this->sanitize_db_response(
            $results,
            'WP Shopify Error - Database query failed when executing: ' .
                $query,
            'query'
        );
    }
	...
	*/
    public function delete_table()
    {
        ...
        if ($this->table_exists($table_name)) {
            ...
            return $this->sanitize_db_response(
                $results,
                'WP Shopify Error - Failed to delete table: ' .
                    $table_name .
                    '. Please clear the plugin cache and try again.',
                'query'
            );
	...
	*/
    public function rename_migration_table()
    {
        ...
        if (
            !$this->table_exists(
                $table_name . WP_SHOPIFY_TABLE_MIGRATION_SUFFIX
            )
        ) {
            return true;
        }
        ...
        return $this->sanitize_db_response(
            $results,
            'WP Shopify Error - Failed to rename migration table back to: ' .
                $table_name .
                '. Please clear the plugin cache and try again.',
            'get_results'
        );
	...
	*/
    public function create_migration_table($table_suffix)
    {
        ...
        if (!$this->table_exists($table_name . $table_suffix)) {
            ...
            $result = $wpdb->query(
                $this->create_table_query($table_name . $table_suffix)
            );
	...
	*/
    public function delete_rows($column_name, $column_value)
    {
        ...
        return $this->sanitize_db_response(
            $results,
            'WP Shopify Error - Failed to delete database rows. Please clear the plugin cache and try again.',
            'query'
        );
    }
	...
	*/
    public function delete_rows_in($column, $ids)
    {
        ...
        $result = $wpdb->get_results($wpdb->prepare($query, $ids));

        return $this->sanitize_db_response(
            $result,
            'WP Shopify Error - Failed to delete database rows by value. Please clear the plugin cache and try again.',
            'get_results'
        );
    }
	...
	*/
    ...
    {
        $col_names_string_single_quotes =
            '`' .
            Utils::convert_to_comma_string_backticks(
                $this->get_only_matching_cols($old_cols)
            ) .
            '`';
	...
	*/
    public function get_columns_as_insert_string($old_cols)
    {
        ...
        $col_names_string_backticks = $this->build_columns_with_backticks(
            $old_cols
        );

        return ' (' .
            $col_names_string_backticks .
            ') SELECT ' .
            $col_names_string_backticks .
            ' FROM ' .
            $table_name;
    ...
    public function get_insert_into_start()
    {
        ...
        return 'INSERT INTO ' . $table_name . WP_SHOPIFY_TABLE_MIGRATION_SUFFIX;
    }

    public function build_insert_into_query($old_cols)
    {
        return $this->get_insert_into_start() .
            $this->get_columns_as_insert_string($old_cols);
    }
    ...
    public function get_lookup_value($item)
    {
        ...
        // id
        if (Utils::has($item, WP_SHOPIFY_SHOPIFY_PAYLOAD_KEY)) {
            return $item->{WP_SHOPIFY_SHOPIFY_PAYLOAD_KEY};
        }
    }
    ...
    public function get_current_items($method_name, $options)
    {
        return $this->$method_name(
            $options['item']->{WP_SHOPIFY_SHOPIFY_PAYLOAD_KEY}
        );
    }
	...
	* Important * WP_SHOPIFY_SHOPIFY_PAYLOAD_KEY is used to build the get_{items}_from_product_id
