	...
	*/
    public function request($method, $url, $body = false, $blocking = false)
    {
        ...
        if (!empty($dep_reason)) {
            return false;
            // error_log(
            //     'WP Shopify Warning: A response includes a resource or property that is deprecated and removed in a later API version: ' .
            //         $dep_reason
            // );
        }
        ...
        if ($api_ver !== WP_SHOPIFY_ADMIN_REST_API_VERSION) {
            return false;
            // error_log(
            //     'WP Shopify Warning: Admin REST API version does not match! Request returned "' .
            //         $api_ver .
            //         '" but app uses "' .
