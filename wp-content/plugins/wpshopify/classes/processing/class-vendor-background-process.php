...
abstract class Vendor_Background_Process extends
    \WP_Shopify\Processing\Vendor_Async_Request
{
    /**
     * Action
     *
    ...
    public function add_meta_to_batch($params)
    {
        if (isset($params['meta']['increment_name'])) {
            $option_name =
                $this->identifier .
                '_meta_' .
                $params['meta']['post_type'] .
                '_' .
                $params['meta']['increment_name'];
        } else {
            $option_name =
                $this->identifier . '_meta_' . $params['meta']['post_type'];
        }
    ...
    public function wp_cron_disabled()
    {
        ...
        if (defined('ALTERNATE_WP_CRON') && ALTERNATE_WP_CRON) {
            return new \WP_Error(
                ...
                sprintf(
                    __(
                        'The ALTERNATE_WP_CRON constant is set to true as of %s. This plugin cannot determine the status of your WP-Cron system.',
                        'wpshopify'
                    ),
        ...
        if (is_wp_error($result)) {
            ...
            return new \WP_Error(
                ...
                sprintf(
                    __('Unexpected HTTP response code: %s', 'wpshopify'),
	...
	*/
    public function save()
    {
        ...
        if (!empty($this->data)) {
            $updated = Options::update(
                $key,
                $this->before_queue_item_save($this->data)
            );
        }
    ...
    protected function lock_process()
    {
        ...
        $lock_duration = property_exists($this, 'queue_lock_time')
            ? $this->queue_lock_time
            : 28; // 1 minute
	...
	*/
    protected function handle()
    {
        ...
        do {
            ...
            if (
                $this->action === 'wps_background_processing_products' ||
                $this->action === 'wps_background_processing_variants' ||
                $this->action ===
                    'wps_background_processing_posts_collections_custom' ||
                $this->action ===
                    'wps_background_processing_posts_collections_smart' ||
                $this->action === 'wps_background_processing_posts_products'
            ) {
  ...
  @return bool
	...
	*/
    protected function time_exceeded()
    {
        $finish =
            $this->start_time +
            apply_filters($this->identifier . '_default_time_limit', 25); // 25 seconds
        $return = false;
